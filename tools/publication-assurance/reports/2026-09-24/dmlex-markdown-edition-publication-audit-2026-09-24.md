...
### F8. Gate defects found and fixed while auditing this work
...
**Exposure window:** 24 to 25 September 2026: both rendered PDFs in reports/2026-09-24/rendered on the markdown-conversion branch of the fork. Nothing was published to docs.oasis-open.org.

**Remediation, verified:** Font sizes from the PDF text layer (PyMuPDF spans, share of the characters outside the footer): v1.0 OS body LiberationSans 10.0pt<sup>S4</sup> 44.6%<sup>S4</sup>, code CourierNewPSMT 9.0pt<sup>S4</sup> 41.1%<sup>S4</sup>, footer 8.0pt<sup>S4</sup>; published body ArialMT 10.0pt<sup>S4</sup> 46.4%<sup>S4</sup>, code Courier 10.0pt<sup>S4</sup> 38.4%<sup>S4</sup>, footer 8.0pt<sup>S4</sup>. 194<sup>S4</sup> pages for v1.0 OS and v1.1 WD01 alike. The layout defects in the new render, a heading left alone above the Appendix D diagram and a code block split from its caption, were fixed by keeping headings and example captions with what follows them.
...
Timestamps are Europe/London. The validation reports (pub-check) are filed alongside this one: dmlex-v1.1-wd01-pub-check-validation-2026-09-24 and dmlex-v1.0-os-markdown-pub-check-validation-2026-09-24. The sources listed under Sources were produced on 25 September 2026, when this record was rebuilt.
...
- **S10.** `sources/print_pdf-c1c2a96.mjs.txt`. Produced by (run from the repository root): `git show c1c2a96:tools/publication-assurance/print_pdf.mjs > tools/publication-assurance/reports/2026-09-24/sources/print_pdf-c1c2a96.mjs.txt`
- **S11.** `sources/docbook2md-c1c2a96.py.txt`. Produced by (run from the repository root): `git show c1c2a96:tools/docbook-to-markdown/docbook2md.py > tools/publication-assurance/reports/2026-09-24/sources/docbook2md-c1c2a96.py.txt`
...
- **S13.** `sources/comparison-captures.json`. Produced by (run from the repository root): `python3 -c "import glob, json; d = 'tools/publication-assurance/reports/2026-09-24/comparison'; h = sorted(glob.glob(d + '/html-*.png')); p = sorted(glob.glob(d + '/pdf-*.png')); print(json.dumps({'html': len(h), 'pdf': len(p), 'total': len(h) + len(p), 'files': [x.rsplit('/', 1)[1] for x in h + p]}, indent=1))" > tools/publication-assurance/reports/2026-09-24/sources/comparison-captures.json`
